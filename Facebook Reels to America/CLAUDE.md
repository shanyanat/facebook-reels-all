...
**Google Flow lives at `flow.google.com` now, and its UI was redesigned (2026-09-11).** `https://labs.google/fx/th/tools/flow` answers **308 → `https://flow.google.com/`**, so `FLOW_URL` points at the new host. The redesign changed what `video_phase.py` has to click, and the same rules apply to the extension's `content/flow.js` (documented at length in that project's CLAUDE.md):

- **Labels are icon ligatures glued to their text** — `addNew project`, `videocamVideo`, `crop_freeFrames`, `crop_9_169:16`. Match by substring/suffix, never equality. The output multiplier is `x1`, not `1x`.
- **`[aria-label="Settings trigger"]` opens the settings panel.** The old "shortest bottom pill matching `/\d+x/`" heuristic no longer matches (`Video · 720p · 8s crop_9_16 x1`). Flow also remembers the panel being left open, and clicking the trigger then closes it — `_settings_panel_open()` is checked first.
- **Video mode defaults to "Ingredients"; the bot needs "Frames".** Only Frames shows the **Start / End** frame slots that the scene image goes into.
- **Upload is: `[aria-label="Add media menu"]` → `Upload` → file chooser, *then* click the Start slot** to open the "Select a frame image" picker, pick the entry matching the filename, then **Add to prompt**. The old `อัปโหลดสื่อ / Upload media` browser is gone, and the Start slot does nothing until the project already holds media — which is why the upload happens first. `_legacy_media_browser_upload()` keeps the old route as a fallback. Clicking the picker entry usually attaches it and closes the picker outright, so `_frame_attached()` — not the presence of an "Add to prompt" button — is what says the step succeeded.
- **There are no `<video>` elements left in Flow**: the grid shows thumbnails and the viewer paints into a `<canvas>` (confirmed with a shadow-DOM- and iframe-piercing search). `_count_clips()` therefore counts `img[alt="Generated video thumbnail"]`; counting `<video>` returned 0 forever, which made every generation look like a 180 s timeout when the clip had in fact been produced.
- **Download is: click the clip tile → `[aria-label="Download media"]` → "720p Original size"** (`_download_via_viewer()`, with the old right-click-the-card path kept as fallback). Take 720p and nothing else automatically — it is the resolution the clip was generated at, so it is already rendered and free, while "1080p Upscaled" re-renders it and "4K Upscaled" spends 50 credits.
...
**`do_archive()` is idempotent — safe to re-run (fixed 2026-07-01).** A successful archive removes the page's `working/` folder once it is empty, so a *second* `py bot.py archive <reel>` (or archiving a reel that has no `working/` files at all — e.g. one generated on another machine) used to crash with `FileNotFoundError` on `working_dir.iterdir()`. Both iterdir loops now guard with `if working_dir.exists() else []`, so re-archiving an already-archived reel is a harmless no-op (it prints "may already be archived" warnings and moves 0 clips). Do not remove these guards. This also protects `do_force_complete()` and the extension's `/api/archive` button, which both call `do_archive()`.

**Accepting a partial reel (e.g. 9/10).** A reel never needs all its scenes to be archived. `do_archive()` has no minimum — it moves whatever `-vdo.mp4` clips exist. `py bot.py force-complete <reel>` is the one-shot partial-accept path: it is exactly `do_archive(reel)` + `do_collect()` + `sweep_captions(page)`, so it archives the finished clips, collects them to `complete/`, and writes `caption.txt` — then the editor queue renders whatever clip count it has. The extension's per-reel **Archive** button also appears for partial (`videos_in_progress`) reels, so a partial can be approved the same way as a full one.
