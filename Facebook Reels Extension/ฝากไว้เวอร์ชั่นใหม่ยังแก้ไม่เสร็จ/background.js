...
// ── Video download renaming ───────────────────────────────────────────────────
// Dual strategy: in-memory Map (fast/sync) + chrome.storage.local (survives SW restarts).
// On SW startup, repopulate the Map from storage so rename works even after a restart.

const _pendingVideoFilenames = new Map();
chrome.storage.local.get(null).then(all => {
    for (const [k, v] of Object.entries(all)) {
        if (k.startsWith('pending_dl_')) {
            const id = parseInt(k.replace('pending_dl_', ''), 10);
            if (!isNaN(id)) _pendingVideoFilenames.set(id, v);
        }
    }
});

chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.action !== 'setPendingVideoFilename') return false;
    const tabId = sender.tab?.id;
    if (!tabId) { sendResponse({ ok: false, error: 'no tabId' }); return true; }
    _pendingVideoFilenames.set(tabId, msg.filename);
    chrome.storage.local.set({ ['pending_dl_' + tabId]: msg.filename }).then(() => {
        console.log(`[bg] setPendingVideoFilename tab=${tabId}:`, msg.filename);
        sendResponse({ ok: true });
    }).catch(e => sendResponse({ ok: false, error: e.message }));
    return true;
});

// Fetch a blob: URL from the page's MAIN world (content scripts can't access page blobs).
// Returns { ok: true, base64: '...' } or { ok: false, error: '...' }.
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.action !== 'fetchBlobAsBase64') return false;
    const tabId = sender.tab?.id;
    if (!tabId) { sendResponse({ ok: false, error: 'no tabId' }); return true; }
...
// Rename any incoming .mp4 download to reel_XXXX-scene-XX-vdo.mp4.
// Strategy:
//   1. Sync lookup in Map by tabId — fast, works when SW is alive
//   2. Async storage scan — fallback for SW restarts or tabId=-1 downloads
//      (only uses scan-fallback when exactly one slot is waiting, to avoid cross-slot mismatch)
chrome.downloads.onDeterminingFilename.addListener((downloadItem, suggest) => {
    const tabId = downloadItem.tabId;
    console.log(`[bg] onDeterminingFilename tabId=${tabId} file=${downloadItem.filename}`);

    // Fast path: synchronous Map lookup
    if (tabId != null && tabId >= 0) {
        const pending = _pendingVideoFilenames.get(tabId);
        if (pending) {
            _pendingVideoFilenames.delete(tabId);
            chrome.storage.local.remove('pending_dl_' + tabId);
            console.log(`[bg] Renaming (sync) tab=${tabId} →`, pending);
            suggest({ filename: pending, conflictAction: 'overwrite' });
            return false; // synchronous suggest — no need for return true
        }
    }

    // Slow path: scan storage (handles SW restart + tabId mismatch / tabId=-1)
    chrome.storage.local.get(null).then(all => {
        const pendingKeys = Object.keys(all).filter(k => k.startsWith('pending_dl_'));

        // Exact key match
        const exactKey = tabId != null && tabId >= 0 ? 'pending_dl_' + tabId : null;
        let pending = exactKey && all[exactKey] ? all[exactKey] : null;
        let matchedKey = pending ? exactKey : null;

        // Scan fallback: only safe when exactly one slot is waiting (avoids cross-slot mismatch)
        if (!pending && pendingKeys.length === 1) {
            matchedKey = pendingKeys[0];
            pending = all[matchedKey];
        }

        if (pending) {
            chrome.storage.local.remove(matchedKey);
            const id = parseInt(matchedKey.replace('pending_dl_', ''), 10);
            if (!isNaN(id)) _pendingVideoFilenames.delete(id);
            console.log(`[bg] Renaming (async) →`, pending);
            suggest({ filename: pending, conflictAction: 'overwrite' });
        } else {
            console.log(`[bg] No pending rename for tabId=${tabId} — using default`);
            suggest();
        }
    }).catch(() => suggest());
