...
- **`content/flow.js`** — Content script on flow.google.com (and legacy labs.google/fx). Drives Google Flow video generation. Selects which scenes still need a video from **disk truth** (`img_on_disk`/`vdo_on_disk`, served by monitor.py), not from contents.json flags — see "Video phase: disk-based selection" below.
...
### Google Flow moved to flow.google.com and redesigned its UI (2026-09-11)

`https://labs.google/fx/th/tools/flow` now answers **308 → `https://flow.google.com/`**. The old manifest matched content scripts on `https://labs.google/fx/*` only, so after the redirect **`flow.js` was never injected**: no `tabReady`, so background never sent `startVideos`, and the tab simply parked on the Flow home page forever without clicking "+ New project". That is the whole symptom — the automation was not stuck, it was never started.

Fixed in three places, and `background.js` now has **one** source for the URL (`PHASE_URL.videos`) because two literals where only one got updated is what produced this: `manifest.json` matches `https://flow.google.com/*` (the legacy `labs.google/fx/*` match is kept), `PHASE_URL.videos` is `https://flow.google.com/`, and the `imagesComplete` handler reads `PHASE_URL.videos` instead of repeating the literal. `host_permissions` already covered the new host via `https://*.google.com/*`, so no new permission is requested. **A manifest change needs the extension reloaded at `chrome://extensions` — the fix does nothing until then.**

The redesigned UI also broke the selectors, so these rules now hold in `flow.js`:

- **Every label is an icon ligature glued to its text** — `videocamVideo`, `crop_freeFrames`, `crop_9_169:16`, `addNew project`. Match by **suffix or substring, never equality**. The old `t === 'Video'` and `t === '1x'` tests both failed; the multiplier is now `x1`, not `1x`.
- **The settings panel is opened by `[aria-label="Settings trigger"]`**, not by hunting for the shortest pill matching `/\dx/` — the pill's text is now `Video · 720p · 8s crop_9_16 x1`, which that regex does not match. Flow also **remembers whether the panel was left open**, and clicking the trigger then *closes* it and the following click lands on the page behind, so `openSettingsPanel()` checks `settingsPanelOpen()` first and only clicks when it is shut.
- **Select "Frames", not "Ingredients".** Video mode defaults to Ingredients (reference images); only Frames puts the **Start / End** frame slots in the compose bar, which is where the scene image goes.
- **Pick the model from the menu, never the trigger.** `[aria-label="Select model family"]` opens it; the trigger button's own text contains the current model, so a text match for `Lite` / `Lower Priority` would re-click the trigger and close the menu. Options read `volume_upVeo 3.1 - Lite [Lower Priority]` — that one is preferred, plain `Lite` is the fallback.
- **Anything matching aspect-ratio text must skip `[aria-label="Settings trigger"]`** — the pill's own text carries `crop_9_16` and clicking it closes the panel.

**The upload path changed completely, and it must run in the page's MAIN world.** The `อัปโหลดสื่อ / Upload media` media browser is gone, and Flow now creates its `<input type=file>` only for the duration of its own click handler (verified: `document.querySelector` finds it *during* the handler and never before or after), so the old "override `.click()` and capture the input" trick has nothing to capture. The replacement is a **drop** — a `DragEvent` carrying a `DataTransfer` onto the Start slot, which Flow ingests into the project's media library.

**That drop does not work from a content script.** Dispatched from the isolated world it silently does nothing: no media reaches the library, and the run then fails at the next step. This is the same wall `injectFileUpload` hit for ChatGPT, and it has the same remedy — `dropSceneImage()` in `flow.js` sends `dropFlowFrame` to background.js, which fetches the PNG from monitor.py, base64s it, and runs the File construction and the drop via `chrome.scripting.executeScript({world: 'MAIN'})`. **Do not "simplify" this back into flow.js.** `host_permissions` already covers `flow.google.com` via `https://*.google.com/*`, so the injection is permitted.
...
**The Start slot must be matched by its ACCESSIBLE NAME, not by `el.textContent` (2026-09-12).** Written while chasing the "same image uploads over and over, no video is ever generated" loop. **Verified:** three places compared the slot's raw text by equality against a label the ligature rule above says can carry an icon prefix, and the terminal twin — which works — matches the accessible name instead. **Not yet confirmed:** that this was the *live* break in that run, because the decisive log line was never read. It is one line: `dropped on body (NO Start slot found)` confirms it, while `dropped on slot "…"` means the slot was found all along and the break is downstream, where the new DIAG dumps name the step. Read that line before building on this section. (`tests/test_slot_matcher.js` pins the matcher against hand-authored stub DOM — the shapes Material buttons take, not a capture of Flow's real markup.) Three places compared the slot's raw text against `['Start','เริ่ม','เริ่มต้น']` with `.includes()` — i.e. **exact equality** — while the label renders with the Material icon glued on (`<span aria-hidden="true">image</span>Start` → `textContent === "imageStart"`). All three therefore missed the slot:

- `background.js` `dropFlowFrame` fell through to `|| document.body`. Flow's page-level drop zone still ingests the PNG, **so the image uploaded and looked fine** — but nothing was attached to the Start frame.
- `flow.js` `openFramePicker()` found neither an exact-`Start` button nor an `[aria-label="Image ingredient"]` chip → 5 dead attempts → `SELECTOR: frame picker did not open` → `reloadForRetry()` → and because `uploadSceneImage()` re-dropped unconditionally, the next attempt uploaded the same file again. Five identical uploads, no generate, then a stop.
- `flow.js` `frameAttached()` returned **true** when that match failed (its fallback is "no empty Start slot visible"), a false pass in the other direction that would happily generate a clip with no start frame.

**This is also why the terminal twin worked while the extension did not**, which had looked inexplicable: `video_phase.py` uses Playwright's `get_by_role("button", name="Start", exact=True)`, and accessible-name computation **ignores `aria-hidden` subtrees**. Same element, same page — Playwright finds it, `textContent` equality does not. **A green terminal run is therefore not evidence that an extension selector is right.** (`_frame_attached()` in the Python twin carries the same raw-text fallback, but the `[aria-label="Image ingredient"]` check short-circuits it once a frame is attached, so it is latent there, not live. It was left alone deliberately — that path is verified working.)

`flow.js` now has `accName(el)` (aria-label, else text with `aria-hidden` subtrees stripped) plus `isStartSlot(el)` / `findStartSlot()`, and `dropFlowFrame` carries the same helpers inlined in its MAIN-world function. `isStartSlot` accepts the accessible name **or** a ligature suffix, anchored at the END and length-capped, so `Start over`, `Get Started`, `Restart`, `End` and `videocamVideo` cannot match. `tests/test_slot_matcher.js` pins all twelve shapes — run `node tests/test_slot_matcher.js` after touching any of it.

Three further changes came with it, so this failure can't hide again:

- **`uploadSceneImage()` is idempotent.** It takes `pid`, records the attached filename in `chrome.storage.local` (`flow_attached_<pid>`, cleared on a fresh run and at phase end), and **skips the drop entirely** when `frameAttached()` and that record already name this scene's file. A retry after `reloadForRetry()` no longer re-uploads — which is what filled the project with duplicate copies of one scene and made `selectPickerFile()` ambiguous (it takes the first visible match, possibly a half-uploaded one).
- **A drop onto an empty slot is accepted as the attachment.** `uploadSceneImage()` samples `findStartSlot()` *before* the drop; if the slot was empty and `frameAttached()` is true afterwards, the only file in it is the one just dropped, so the picker is skipped rather than clicked (clicking the chip can clear the slot again). Scene 2 onward starts with the previous scene's frame still in the slot, so `slotWasEmpty` is false there and the normal picker route runs.
- **Every miss now dumps candidates.** `dumpComposeButtons()` logs `txt` / `acc` / `aria` / position for lower-viewport buttons when the slot or the attach fails; `selectPickerFile()` lists the picker entries it *did* see (text, aria-label, `img alt`) and also matches the filename in those attributes, not just in text; and `dropFlowFrame` returns `dropped on slot "…"` vs `dropped on body (NO Start slot found) | lower-viewport buttons: …`. **That one log line is the fastest diagnosis of this whole class of bug** — check it first next time.

**A synthetic click must land on the element under the cursor, not on the container (2026-09-12, round 2).** After the accessible-name fix the symptom moved: the image uploaded and the picker was reached, but nothing was ever added to the prompt — so `uploadSceneImage()` threw at the attach step, and because `fillVideoPrompt()` and `clickGenerate()` run *after* it, **the video prompt was never typed at all**. That is not a second bug; "the prompt isn't being added" is the throw, seen from the outside. Any report of "no prompt and no video" means the run died in `uploadSceneImage()` — read upward from there, never at `fillVideoPrompt`.

Three candidates were live at once, and the reason they couldn't be told apart is that `selectPickerFile()` logged `Selected in picker: <file>` **immediately after the click, unconditionally** — it reported the action, not the effect. Every step in the attach chain now asserts its own postcondition instead:

- **Container click (the leading candidate).** `dispatchPointerClick()` dispatched its whole sequence on the matched element. A picker entry is a wrapper around a clickable tile, and React resolves a synthetic click by walking the fiber path **upward from the event target** — so a handler on an inner child never runs, the entry was never selected, and "Add to prompt" stayed greyed for ever. It now dispatches on `document.elementFromPoint(centre)` when that hit is `el` or inside it, which is what Playwright's `.click()` effectively does (it presses a coordinate). Events still bubble to `el`, so this is strictly more faithful; a hit outside `el` (an overlay) falls back to the old behaviour. **This is the same class of bug as the accessible-name one: a Playwright high-level action is not equivalent to a naive synthetic dispatch, so the twin passing proves nothing about the extension.**
- **Partial-copy selection.** `findAddToPromptBtn()` already documented this ("the still-uploading (47%) copy") — and the retry loop was *manufacturing* it, re-dropping the same PNG on every attempt until the project held a dozen copies of one scene under one name, of which `selectPickerFile()` took the first. `selectPickerFile()` now tries up to four matching copies, moving on when one yields no selection signal.
- **Class-only disabled.** `isEnabled()` knows only `disabled` and `aria-disabled`. If Flow greys the button with a CSS class, the button is returned as enabled and clicked uselessly. `clickAddToPrompt()` now dumps `describeButton()` — `disabled`, `aria-disabled`, `class`, computed `pointer-events`, `opacity` — both when the button never enables and after 3 dead clicks.

**Never re-upload a file the project already holds.** `flow_dropped_<pid>` in `chrome.storage.local` lists the scene files uploaded into the current Flow project; it is cleared only when a fresh run clicks New project (a genuinely empty library) and at phase end. A retry after `reloadForRetry()` therefore skips the drop and goes straight to the picker. This is what the user actually sees and reports ("it keeps uploading the same images"), and it is self-reinforcing: every extra copy makes the partial-copy trap above likelier, so the loop got *worse* each cycle rather than converging. The skip is only an optimisation — if step 3 finds the recorded file unselectable after all, it calls `unmarkDropped()`, closes the picker, uploads once more and retries the picker, so a wrong record can never loop.

**`mediaPanelOpen()` answers true for any stray Add-to-prompt / Upload-media button**, not just for the open dialog, so `openFramePicker()` can return true without ever clicking the slot. It now logs `Frame picker reported already open` when that happens on the first check, and `selectPickerFile()`'s dump distinguishes "entries listed, none ours" from "(none — the picker may not actually be open)".

**Verified:** the file parses, `tests/test_slot_matcher.js` passes, and the postconditions are now asserted rather than assumed. **Not verified:** which of the three candidates was live, and the `elementFromPoint` retarget against Flow's real markup — neither can be checked from here (see the Chrome-151 note below). The next run's own log names it: which selection signal fired (`frame attached` / `Add-to-prompt enabled` / `aria-selected`), or a `describeButton` dump, or `(none …)`.

**"Is there any text?" is not "did our prompt land?" — the prompt fill had a double false-pass (2026-09-12, round 3).** Reported symptom: the image reaches the Start slot but the compose bar stays on `What do you want to create?`, no prompt, no clip. **Slate renders its placeholder in a child span INSIDE the editable**, so both sides of the verification read a 27-character placeholder as success:

- `fillSlate` in `background.js` ended `return 'ok:' + editorEl.textContent.trim().length` — `ok:27` for an insert that did nothing.
- `fillVideoPrompt` then tested `actual.length > 0` on the same raw `textContent`, logged **`Prompt filled (1782 chars via Slate main-world)`**, and returned.

So the run *reported success on an empty box*, its fallback never executed, and the failure surfaced up to 210 s later as something unrelated — a missing Generate button, or a clip that ignored the prompt. **Any "filled/attached/selected" log in this file must compare against what was intended, never against "something is there".** That mistake has now appeared three times: `selectPickerFile` logging the click instead of the effect, `frameAttached()`'s negative fallback, and this.

`editorText()` strips `[data-slate-placeholder]` / `[data-placeholder]` (and reads `.value` for textarea/input); `promptFilled(text)` collapses whitespace on both sides and requires ≥50 % of the intended length **and** its leading 20 characters. `tests/test_prompt_verify.js` pins twelve cases, the first being the placeholder-only box. `fillSlate` strips the placeholder before reporting its length too.

**The fallback was also missing the only selectors that could have worked.** It tried `textarea, input[type="text"]` only — but `waitForCompose()` accepts `textarea` **or** `[contenteditable="true"]` **or** `[role="textbox"]`, and Flow's compose bar is one of the latter two. A contenteditable compose bar therefore had *no* working path once Slate's own API missed. `PROMPT_SELECTORS` / `findPromptEditors()` now mirror the terminal twin's list (lowest element on the page first — that is the compose bar), and for a contenteditable the fallback focuses it and uses `document.execCommand('insertText')`: a browser editing command, so the input events it fires are trusted. It deliberately does **not** touch selection — no `selectAll`, no `addRange` — which is the documented Slate crash; `focus()` alone puts the caret in. That path is last, after Slate's API and the native value setter. **This is the third instance of the same asymmetry: Playwright's `.fill()` focuses, clears and dispatches a real input sequence, so the twin passing says nothing about `el.value = x`.**

**A missing prompt is now fatal, and re-checked at the last moment.** `fillVideoPrompt` throws `SELECTOR: could not type the video prompt into the compose bar` instead of logging a warning and letting `clickGenerate()` run on an empty box — `SELECTOR:` so it retries after a reload and stops loudly rather than burning the scene's fail budget. `runVideos` then calls `promptFilled()` **again immediately before Generate**, because attaching the frame re-renders the compose bar. Generating with an empty box is never acceptable: Flow either refuses (which reads as "Generate button not found") or returns a clip that ignored the prompt and still counts as that scene's deliverable. `clickGenerate()` also dumps `describeButton()` for every lower-viewport button when the arrow isn't found, since "not found" usually means "found but disabled".

**Do not record which step the run dies at from screenshots.** A screenshot of the compose bar holding the image with an empty prompt box is equally consistent with (a) the attach working and the prompt failing, and (b) the user demonstrating the intended procedure while the bot is still stuck at the picker. The prompt path was broken as written either way, which is why it was fixed; round 2's assertions (`pickerSelectionSignal`, `describeButton`) are what will adjudicate the attach step. The one line that settles it: a `Prompt filled` whose character count matches the brief's prompt, versus the new `DIAG: prompt never landed` dump.

**Downloading a clip: HOVER the tile, never click it (2026-09-12, round 4).** Corrected by the user, who drives this UI by hand: the procedure is **hover the newest clip tile → the ⋮ appears on the tile → click ⋮ → "Download" → "720p"**. Clicking the tile opens Flow's full-screen viewer, and "the entire display will be incorrect" — the layout that leaves behind is not one the rest of the run can drive. The previous `downloadClipViaViewer()` made that click its **first step**, so every download began by breaking the page. It is deleted, not merely bypassed; `downloadNewestClip()` replaces it. **Never reintroduce a tile click.**

- `dispatchHover()` sends `pointerover/enter → mouseover/enter → pointermove/mousemove` with `buttons: 0` and **no** down/up/click. A hover is not a click with fewer steps — any `pointerdown` here opens the viewer.
- The ⋮ is scoped by **geometry**, not by climbing the DOM: `overTile()` keeps only buttons whose centre lies within the tile's rect + 24 px. The header carries its own ⋮ buttons (screenshot: x≈259 and x≈1129 at y≈27), and a parent walk that reached a page-wide wrapper would have found them; geometry also keeps a portal-rendered hover overlay — which has no tile ancestor at all — in scope. `tests/test_tile_menu.js` pins ten positions from the user's own screenshot, including both header buttons and a second-row tile.
- **720p or nothing.** `findMenuItem(/720/, /4K|GIF|1080|Upscal/i)` — a missing 720p is a hard stop, never a click on whatever else is offered. 720p is the resolution the clip was generated at, so it is already rendered and free; "1080p Upscaled" re-renders it and "4K Upscaled" spends 50 credits.
- `expectClipDownload` is armed **immediately before** the 720p click, and the menu is Escaped afterwards; if anything did manage to open the viewer, `leaveClipViewer()` gets out so the next scene starts from the normal layout.

`phases/video_phase.py`'s `_download_via_viewer()` still uses the click-the-tile route and is **left alone on purpose**: it is verified working there (Playwright's clicks are trusted and `leaveClipViewer()` gets back out), and swapping a proven download path for an unverified one is a regression risk. If the terminal bot ever shows the same broken layout, port `downloadNewestClip()` across.

**A failed download must not silently regenerate the clip.** The old caller ended its no-route branch with `continue`, which sent the scene back through upload → prompt → Generate — spending a second generation on a clip that already existed, every retry. It now throws `SELECTOR: could not start the clip download (hover tile → ⋮ → Download → 720p)`, so the systemic bucket handles it: retry after reload, cap 5, stop loudly with the scene intact. The legacy `<video>`-based right-click and streaming-URL routes are kept as fallbacks *below* the hover menu but are unreachable on the current UI, which has no `<video>` elements.
...
**There are no `<video>` elements left anywhere in Flow.** The grid shows thumbnails and the clip viewer paints into a `<canvas>` — verified by a shadow-DOM- and iframe-piercing search that found zero `<video>` nodes even while a googlevideo.com stream was loading. Two consequences:

- **Clip detection** counts `img[alt="Generated video thumbnail"]` instead (`countVideoClips()`, max'd with the old `<video>` count for the legacy UI). This is what made every generation look like a timeout: the clips were generating fine, `document.querySelectorAll('video').length` just stayed 0 forever.
- **Saving the clip** goes through Flow's own download, renamed in flight. There is no URL to fetch: Flow's "Download media" builds the file in the page and hands the browser a **`blob:` URL**, which cannot be re-requested (verified — a second request to it fails outright, and the googlevideo URL behind the canvas player is never exposed to the DOM). So `downloadClipViaViewer()` opens the newest tile, arms background.js's `expectClipDownload`, and clicks **Download media → 720p Original size**; background.js's `chrome.downloads.onDeterminingFilename` renames that download to `reel_XXXX-scene-NN-vdo.mp4`, which is exactly what `monitor.py`'s `DownloadsHandler` files into `working/`, and `waitForFileInWorking()` confirms it landed. **Pick 720p and nothing else automatically** — it is the resolution the clip was generated at, so it is already rendered and free, while "1080p Upscaled" re-renders and "4K Upscaled" costs 50 credits. `DownloadItem`s carry no tabId, so only **one rename is armed at a time**: a second slot asking while one is armed is refused and retries its scene, and an arm that times out leaves the file under Flow's own name where the handler ignores it — a visible timeout rather than a clip silently saved as the wrong scene. Do not "simplify" this by arming several at once.
...
**Preferred: Git.** The whole workspace (`Facebook Reels All`) is one repo (remote `origin/master`, github.com/shanyanat/facebook-reels-all). To ship a code change: on the source machine **bump `manifest.json`'s `version`** (see below) → `git add` the changed file(s) → `git commit` → `git push`; on the target machine `git pull`. Then in Chrome on the target: `chrome://extensions` → **Reload** the extension so it picks up the new code (a content-script/JS edit needs this reload to take effect). `data/`, `pages/`, `complete/`, and `chrome-bot/` are **gitignored**, so a pull never overwrites a machine's local data/logins.

**Always bump `manifest.json`'s `version` when you change ANY extension file — it is the only way to tell which code a machine is actually running.** The scheme is the date of the change, `YYYY.M.D` (e.g. `2026.9.13`); Chrome accepts up to four dot-separated integers 0–65535. Chrome prints that version under the extension's name on `chrome://extensions`, so the user can compare it to the version on disk and see at a glance whether they forgot the Reload. It sat frozen at `1.0` from the first commit until 2026-09-13, which is exactly why a machine running week-old `flow.js` against the redesigned Google Flow was indistinguishable from an up-to-date one. That is the leading suspect (not a reproduced diagnosis) when attaching a scene image raises Flow's own **"failed to upload"** toast: code between `fd69ed5` and `072264c` injects fine on `flow.google.com` but builds its `DataTransfer` in the isolated world, where Flow's drop handler never sees it. Rule the other causes out with `py bot.py doctor` before assuming it. `py bot.py doctor` (in `Facebook Reels to America/`) reads this version off disk and tells the user what Chrome must show.
