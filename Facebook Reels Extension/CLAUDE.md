...
**The upload path changed completely.** The `อัปโหลดสื่อ / Upload media` media browser is gone, and Flow now creates its `<input type=file>` only for the duration of its own click handler (verified: `document.querySelector` finds it *during* the handler and never before or after). A content script runs in the isolated world with its own `HTMLInputElement.prototype`, so the old "override `.click()` and capture the input" trick can never see that call. `uploadSceneImage()` therefore: (1) **drops** the file — a synthetic `DragEvent` carrying a `DataTransfer` onto the Start slot, which Flow ingests into the project's media library and which needs no page-world access; (2) waits for the `<n>%` tile progress to clear; (3) clicks the **Start** slot, which opens the **"Select a frame image"** picker (it does nothing while the project has no media, which is why it runs *after* the drop); (4) clicks the entry matching the filename — never trusting the picker's preselection, since by scene 5 the project holds five images; (5) clicks **Add to prompt**, which survived the redesign unchanged.

`findComposePlusButton()` also gained a guard: its container walk could reach an ancestor holding the top nav, making the left-most button **Home** — clicking that navigated out of the project. It now prefers `[aria-label="Add ingredients to the prompt box"]` and otherwise keeps only buttons on the compose row.
...
**Verified so far:** the injection/URL fix and the whole selector chain up to and including download were proven end-to-end through the terminal twin (`phases/video_phase.py`), which drives the same UI and now generates and saves a clip successfully. The extension's own path carries the same logic but has **not** been run end-to-end here — the URL capture through `chrome.downloads` in particular is implemented, not yet exercised.
